...
/*   Updated: 2017/04/07 05:29:40 by nvarela          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

/* Calculates the offset for the x and y points to obtain the 
* isomatric perspective */
...
static void	ft_cal_whith_z(t_point **point, t_map *map, int i, int j)
{
	...
	while (i < map->s_map)
	{
		while (j < map->sline)
		{
			...
			map->max_x = map->max_x < point[i][j].xi ? point[i][j].xi : map->max_x;
			map->max_y = map->max_y > point[i][j].yi ? point[i][j].yi : map->max_y;
			j++;
		}
		j = 0;
		i++;
...
/*recalculate the max_y and y point whith max_y*/
...
void		ft_search_coord(t_map *map)
{
	...
	if (map->pt[0][map->sline - 1].xi > map->xsize &&
		...
		map->xpos = map->pt[0][map->sline - 1].x - (map->xsize / 2);
}
